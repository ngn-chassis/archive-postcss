...
class ChassisGenerator {
	...
	generate (root, atRule, line, cfg) {
		let element = cfg.args[0]
		
		if (this.elements.hasOwnProperty(element)) {
			this.elements[element](root, atRule, cfg)
			return
		}
		
		console.error(`Chassis spec stylesheet line ${line.line}, column ${line.column}: Element "${element}" not found.`)
	}
